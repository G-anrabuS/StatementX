...
class AICoachService:
    ...
    async def get_or_generate_coach_insights(
        ...
        statement = (
            db.query(Statement).filter(Statement.statement_id == statement_id).first()
        )
        ...
        if statement and isinstance(statement.raw_ai_output, dict):
            ...
            if cached_insights and isinstance(cached_insights, dict):
                ...
                try:
                    ai_recommendations = [
                        AIRecommendationItem(**rec) for rec in cached_recs
                    ]
        ...
        if not ai_summary:
            ...
            if (
                ...
                and highest_spending_category != "Unclassified_Miscellaneous"
            ):
                highest_amount = category_breakdown.get(highest_spending_category, 0)
                pct_of_expense = (
                    (highest_amount / total_expense * 100) if total_expense > 0 else 0
                )
                ...
                static_recommendations.append(
                    AIRecommendationItem(
                        ...
                        target_category=(
                            ...
                            if "netflix" in sub.vendor.lower()
                            or "steam" in sub.vendor.lower()
                            else "Utilities & Services"
                        ),
                    )
                )
            ...
            for anom in anomalies:
                impact = (
                    "High"
                    if anom.type == "high_value"
                    or anom.type == "duplicate"
                    and anom.amount > 1000
                    else "Medium"
                )
                static_recommendations.append(
                    AIRecommendationItem(
            ...
            if settings.GEMINI_API_KEY:
                try:
                    ...
                    debit_transactions_sorted = sorted(
                        debit_transactions, key=lambda t: float(t.debit), reverse=True
                    )
                    ...
                    logger.error(
                        f"Gemini AI coach refinement failed (using static fallback): {e}"
                    )
            ...
            if statement and ai_summary and ai_recommendations:
                try:
                    ...
                    logger.error(
                        f"Failed to cache AI insights in database: {commit_err}"
                    )
                    db.rollback()
        ...
        if statement and existing_chunks == 0 and settings.GEMINI_API_KEY:
            try:
                logger.info(
                    f"Generating comprehensive financial thesis for statement {statement_id}..."
                )
                ...
                for idx, t in enumerate(all_txns, 1):
                    debit_str = (
                        f"Debit: INR {float(t.debit):.2f}"
                        if float(t.debit) > 0
                        else "Debit: 0.00"
                    )
                    credit_str = (
                        f"Credit: INR {float(t.credit):.2f}"
                        if float(t.credit) > 0
                        else "Credit: 0.00"
                    )
                ...
                thesis_data = FinancialThesisResponse.model_validate_json(
                    thesis_res.text
                )
                ...
                try:
                    embed_res = await loop.run_in_executor(
                        ...
                        lambda: client.models.embed_content(
                            model="text-embedding-004", contents=sections_texts
                        ),
                    )
                    embeddings_list = [emb.values for emb in embed_res.embeddings]
                except Exception as emb_err:
                    logger.warning(
                        f"Failed to embed thesis sections (will fallback to fuzzy local search): {emb_err}"
                    )

                for sec, emb_val in zip(thesis_data.sections, embeddings_list):
                    db.add(
                        StatementThesisChunk(
                ...
                db.commit()
                logger.info(
                    f"[SUCCESS] Financial thesis generated and indexed into vector DB for statement {statement_id}!"
                )
            except Exception as thesis_err:
                logger.error(
                    f"Failed to generate and cache financial thesis: {thesis_err}"
                )
                db.rollback()
